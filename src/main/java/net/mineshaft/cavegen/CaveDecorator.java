...
public class CaveDecorator {
    ...
    public static void placeIcePatch(ChunkPrimer primer, int centerX, int centerY, int centerZ, IBlockState primary, IBlockState secondary, IBlockState rare, Random rand) {
        for (int x = -2; x <= 2; x++) {
            for (int y = -2; y <= 2; y++) {
                for (int z = -2; z <= 2; z++) {
                    if (!(Math.abs(z) == 2 && Math.abs(x) == 2) && !(Math.abs(z) == 2 && Math.abs(y) == 2) && !(Math.abs(x) == 2 && Math.abs(y) == 2)) {
                        if(y+centerY>0 && y+centerY<244) {
