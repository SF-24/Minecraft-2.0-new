    ...
    public void setupTerrain(Entity viewEntity, double partialTicks, ICamera camera, int frameCount, boolean playerSpectator)
    {
        ...
        if (!flag && this.displayListEntitiesDirty && !Shaders.isShadowPass)
        {
            ...
            else
            {
                ...
                // Chunk square, replaced by a circle Mineshaft
                for (int dz = -this.renderDistanceChunks; dz <= this.renderDistanceChunks; ++dz)
                {
...
//                        }
            ...
            while (!deque.isEmpty())
            {
                ...
                {
                    // Ignore tile entities far away. Just under 8 chunks. Mineshaft
//                    if((d3-viewEntity.getPosition().getX())*(d3-viewEntity.getPosition().getX())+(d5-viewEntity.getPosition().getZ())*(d5-viewEntity.getPosition().getZ())<16384) {
...
//                    }
    ...
    public void renderSky(float partialTicks, int pass)
    {
        if (Reflector.ForgeWorldProvider_getSkyRenderer.exists())
        {
            ...
            if (object != null)
            {
                Reflector.callVoid(object, Reflector.IRenderHandler_render, new Object[0], Float.valueOf(partialTicks), this.theWorld, this.mc);
                return;
            }
        }
...
//    private void markBlocksForUpdateOptimised(int minX, int minY, int minZ, int maxX, int maxY, int maxZ)
//    {
//        int chunkMinX = (minX >> 4); // Divide by 16
//        int chunkMinZ = (minZ >> 4); // Divide by 16
//        int chunkMaxX = (maxX >> 4); // Divide by 16
//        int chunkMaxZ = (maxZ >> 4); // Divide by 16
//
//        for(int cx=chunkMinX; cx<=chunkMaxX; cx++) {
//            for(int cz=chunkMinZ; cz<=chunkMaxZ; cz++) {
//                RenderChunk chunk = viewFrustum.getRenderChunk(cx<<4,0,cz<<4);
//                if(chunk!=null&&!chunk.isNeedsUpdate()) {
...
//    }
//
//    private void addToQueue(RenderChunk chunk) {
//        int nextHead = (chunkUpdateHead + 1) % 50 /* Queue size */;
//        if(nextHead!=chunkUpdateTail) {
//            // Not full:
//            chunkUpdateQueue[chunkUpdateHead]=chunk;
//            chunkUpdateHead=nextHead;
//        }
//        // If overflow => drop.
//    }
    ...
    public void broadcastSound(int soundID, int x, int y, int z, int data)
    {
        switch (soundID)
        {
            ...
            case 1018:
                ...
                {
                    double d0 = (double)x - this.mc.getRenderViewEntity().posX;
                    double d1 = (double)y - this.mc.getRenderViewEntity().posY;
                    double d2 = (double)z - this.mc.getRenderViewEntity().posZ;
