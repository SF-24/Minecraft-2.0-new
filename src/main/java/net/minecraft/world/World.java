...
public abstract class World implements IBlockAccess {
    ...
    public boolean isBlockLoaded(int x, int y, int z, boolean allowEmpty) {
        return !this.isValid(x, y, z) ? false : this.isChunkLoaded(x >> 4, z >> 4, allowEmpty);
    }
        ...
        if (!this.isValid(pos)) {
            ...
            if (iblockstate == null) {
                ...
                if (!this.isRemote && (flags & 1) != 0) {
                    this.notifyNeighborsRespectDebug(pos.getX(), pos.getY(), pos.getZ(), iblockstate.getBlock());
    ...
    public boolean setBlockState(int x, int y, int z, IBlockState newState, int flags) {
        if (!this.isValid(x, y, z)) {
            ...
            Block block = newState.getBlock();
            IBlockState iblockstate = chunk.setBlockState(x, y, z, newState);
            ...
            if (iblockstate == null) {
                ...
                if (!this.isRemote && (flags & 1) != 0) {
                    this.notifyNeighborsRespectDebug(x, y, z, iblockstate.getBlock());

                    if (block.hasComparatorInputOverride()) {
                        this.updateComparatorOutputLevel(x, y, z, block);
                    }
                } else if (!this.isRemote && (flags & 16) == 0) {
    ...
    public boolean setBlockToAir(int x, int y, int z) {
        return this.setBlockPrimitive(x,y,z, Blocks.air, 3);
    }
    ...
    public boolean destroyBlock(int x, int y, int z, boolean dropBlock) {
        ...
        {
            this.playAuxSFX(2001, x,y,z, Block.getStateId(iblockstate));

            if (dropBlock)
            {
                block.dropBlockAsItem(this, x,y,z, iblockstate, 0);
            }

            return this.setBlockState(x,y,z, Blocks.air.getDefaultState(), 3);
        }
    }
...
//                BlockPos pos = new BlockPos(x,y,z);
                ...
                if (!this.isRemote && (flags & 1) != 0) {
                    BlockPos pos = new BlockPos(x, y, z);
                    this.notifyNeighborsRespectDebug(x, y, z, block);
    ...
    public void notifyNeighborsRespectDebug(int x, int y, int z, Block blockType) {
        if (this.worldInfo.getTerrainType() != WorldType.DEBUG_WORLD) {
            this.notifyNeighborsOfStateChange(x, y, z, blockType);
        }
    }
    ...
    public void markBlockForRenderUpdate(int x, int y, int z) {
        this.markBlockRangeForRenderUpdate(x,y,z,x,y,z);
    }
    ...
    public void notifyNeighborsOfStateChange(int x, int y, int z, Block blockType) {
        this.notifyBlockOfStateChange(x + 1, y, z, blockType);
        this.notifyBlockOfStateChange(x - 1, y, z, blockType);
        this.notifyBlockOfStateChange(x, y - 1, z, blockType);
        this.notifyBlockOfStateChange(x, y + 1, z, blockType);
        this.notifyBlockOfStateChange(x, y, z + 1, blockType);
        this.notifyBlockOfStateChange(x, y, z - 1, blockType);

        // observer:
        this.updateObservingBlocksAt(x, y, z, blockType);
    }

    @Deprecated
    public void notifyNeighborsOfStateChange(BlockPos pos, Block blockType) {
        notifyNeighborsOfStateChange(pos.getX(), pos.getY(), pos.getZ(), blockType);
    }
    ...
    public void notifyBlockOfStateChange(BlockPos pos, final Block blockIn) {
        notifyBlockOfStateChange(pos.getX(), pos.getY(), pos.getZ(), blockIn);
    }
    ...
    public void notifyBlockOfStateChange(int x, int y, int z, final Block blockIn) {
        if (!this.isRemote) {
            ...
            try {
                iblockstate.getBlock().onNeighborBlockChange(this, x, y, z, iblockstate, blockIn);
            } catch (Throwable throwable) {
    ...
    public int getLightFromNeighbors(BlockPos pos) {
        if(pos==null) {
            MineshaftLogger.logError("Getting light from neighbours with null position!");
        }
    ...
    public int getBlockId(int x, int y, int z) {
        return this.getChunkFromBlockCoords(new BlockPos(x, y, z)).getBlockId(x, y, z);
    }
    ...
    public void playRecord(int x, int y, int z, String name) {
        for (int i = 0; i < this.worldAccesses.size(); ++i) {
            ((IWorldAccess) this.worldAccesses.get(i)).playRecord(name, x,y,z);
        }
    }
...
//    }
    ...
    public void playAuxSFX(int p_175718_1_, int x, int y, int z, int p_175718_3_) {
        this.playAuxSFXAtEntity((EntityPlayer) null, p_175718_1_, x, y, z, p_175718_3_);
    }

    @Deprecated
    public void playAuxSFXAtEntity(EntityPlayer player, int sfxType, BlockPos pos, int p_180498_4_)
    {
        playAuxSFXAtEntity(player, sfxType, pos.getX(),pos.getY(),pos.getZ(), p_180498_4_);
    }

    public void playAuxSFXAtEntity(EntityPlayer player, int sfxType, int x, int y, int z, int p_180498_4_)
    {
        try
        {
            for (int i = 0; i < this.worldAccesses.size(); ++i)
            {
                // TODO: CHECK FOR BUGS!!!!
                ((IWorldAccess)this.worldAccesses.get(i)).playAuxSFX(player, sfxType, x,y,z, p_180498_4_);
            }
        }
        catch (Throwable throwable)
        {
            ...
            crashreportcategory.addCrashSection("Block coordinates", CrashReportCategory.getCoordinateInfo(x,y,z));
    ...
    public void updateComparatorOutputLevel(BlockPos pos, Block blockIn) {
        updateComparatorOutputLevel(pos.getX(),pos.getY(),pos.getZ(), blockIn);
    }

    public void updateComparatorOutputLevel(int x, int y, int z, Block blockIn)
    {
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
        {
            ...
            if (this.isBlockLoaded(x,y,z))
            {
                ...
                else if (iblockstate.getBlock().isNormalCube())
                {
                    ...
                    y+=enumfacing.getFrontOffsetY();
                    z+=enumfacing.getFrontOffsetZ();
                    iblockstate = this.getBlockState(x,y,z);
