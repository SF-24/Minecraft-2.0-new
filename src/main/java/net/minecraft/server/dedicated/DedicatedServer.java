...
public class DedicatedServer extends MinecraftServer implements IServer
{
    ...
    protected boolean startServer() throws IOException
    {
        ...
        else
        {
            ...
            logger.info("Starting Minecraft server on " + (this.getHostname().length() == 0 ? "*" : this.getHostname()) + ":" + this.getPort());

            try
            {
                this.getNetworkSystem().addLanEndpoint(inetaddress, this.getPort());
            }
...
//        playerSnooper.addClientStat("whitelist_count", Integer.valueOf(this.getConfigurationManager().getWhitelistedPlayerNames().length));
    ...
    public boolean isBlockProtected(World worldIn, BlockPos pos, EntityPlayer playerIn)
    {
        ...
        else if (this.getConfigurationManager().getOppedPlayers().getKeys().length>0) // TODO: CHECK FOR BUGS
        {
            return false;
        }
