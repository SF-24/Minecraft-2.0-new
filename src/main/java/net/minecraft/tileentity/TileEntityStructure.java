        ...
        else
        {
            ...
            else
            {
                ...
                if (structureboundingbox.maxX - structureboundingbox.minX > 1 && structureboundingbox.maxY - structureboundingbox.minY > 1 && structureboundingbox.maxZ - structureboundingbox.minZ > 1)
                {
                    ...
                    this.markDirty();
                    IBlockState iblockstate = this.worldObj.getBlockState(blockpos);
//                    this.worldObj.markBlockForUpdate(blockpos, iblockstate, iblockstate, 3);
                    this.worldObj.markBlockForUpdate(blockpos);
                    return true;
                }
                else
                {
    ...
    {
        return new ChatComponentTranslation("structure_block.hover." + this.mode.modeName, this.mode == Mode.DATA ? this.metadata : this.name);
    }
